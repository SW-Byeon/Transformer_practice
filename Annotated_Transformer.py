...
class EncoderDecoder(nn.Module):
    ...
    Base for this transformer and many other models.
    """
...
class SublayerConnection(nn.Module):
    """
    A residual connenction followed by a layer norm.
...
#and The second is a simple, position-wise fully connencted feed-forward network.
...
class Decoder(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask):
        ...
        return self.norm(x)
#Each decoder layer has three sub-layer; self-attn, src-attn, and feed-forward.
#Second sub-layer performs multi-head attention over the output of the encoder stack.
#We employ residual connections around each of the sub-layer, followed by layer normalization.
...
#masking
def subsequent_mask(size): #triangle shape
...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout, max_len=5000):
        ...
        pe = pe.unsqueeze(0)
        self.register_buffer("pe", pe)#GPU연산은 가능하지만 optimizer가 업데이트 하지않는다.
...
#Training Data: standard WMT 2014 English-German dataset consisting of about 4.5 million sentence pairs.
#Optimizer: Adam
...
def example_simple_model():
    ...
    for epoch in range(20):
        ...
        run_epoch(data_gen(V, batch_size, 20),
                  ...
                  mode="train")
        model.eval()
        run_epoch(data_gen(V, batch_size, 5),
                  model,
                  SimpleLossCompute(model.generator, criterion),
                  DummyOptimizer(),
                  DummyScheduler(),
...
# using the Multi30k German-English Translation task.
# We also show how to use multi-gpu processing to mak it really fast.

# Data Loading
# We will load the datset using torchtext and spacy for tokenization
...
def train_worker(gpu,
    ...
    train_dataloader, valid_dataloader = create_dataloaders(gpu,
                                                            vocab_src,
                                                            vocab_tgt,
                                                            spacy_de,
                                                            spacy_en,
